...
class ApiService {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      onError: (error, handler) async {
        ...
        if (error.response?.statusCode == 401 && _storageService != null) {
          ...
          if (alreadyRetried) {
            debugPrint('[ApiService] Request already retried, forcing logout');
  ...
  Future<ApiResponse<void>> attachPlaceToTrip(
    ...
    try {
      ...
      final response = await _dio.post('/trips/$tripId/places', data: {
        'placeId': placeId,
  ...
  Future<ApiResponse<NotificationsPayload>> getUnifiedNotifications({
    ...
    try {
      ...
      final response =
          await _dio.get('/users/me/notifications', queryParameters: queryParams);
      debugPrint(
          '[ApiService] Get unified notifications response: ${response.statusCode}');
      if (response.data['success'] && response.data['data'] != null) {
        final data = response.data['data'] as Map<String, dynamic>;
        final items = (data['items'] as List<dynamic>?)
                ?.map((e) => UnifiedNotificationItem.fromJson(
      ...
      debugPrint(
          '[ApiService] Get unified notifications unexpected error: $e');
      return ApiResponse<NotificationsPayload>(
        success: false,
        error: 'An unexpected error occurred',
      );
  ...
  Future<ApiResponse<int>> getUnreadNotificationCount() async {
    try {
      ...
      debugPrint(
          '[ApiService] Get unread count DioException: ${e.message}');
      return ApiResponse<int>(
        success: false,
        error: e.response?.data['error'] ?? 'Network error occurred',
      );
  ...
  Future<ApiResponse<void>> markAllNotificationsRead() async {
    try {
      ...
      debugPrint(
          '[ApiService] Mark all read response: ${response.statusCode}');
      return ApiResponse<void>(
      ...
      debugPrint(
          '[ApiService] Mark all read DioException: ${e.message}');
      return ApiResponse<void>(
        success: false,
        error: e.response?.data['error'] ?? 'Network error occurred',
      );
  ...
  Future<ApiResponse<void>> markNotificationRead(String notificationId) async {
    try {
      ...
      final response =
          await _dio.put('/users/me/notifications/$notificationId/read');
      debugPrint(
      ...
      debugPrint(
          '[ApiService] Mark notification read unexpected error: $e');
      return ApiResponse<void>(
        success: false,
        error: 'An unexpected error occurred',
      );
  ...
  Future<ApiResponse<Map<String, dynamic>>> getPost(
    ...
    try {
      ...
      final response =
          await _dio.get('/posts/$entityType/$entityId');
