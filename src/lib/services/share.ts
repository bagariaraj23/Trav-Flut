...
async function incrementShareCount(entityType: EntityType, entityId: string, tx: Prisma.TransactionClient): Promise<void> {
  if (entityType === 'TRIP_FINAL_POST') {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (redis) {
      await redis.set(
        ...
        { ex: expiresAt ? Math.floor((expiresAt.getTime() - Date.now()) / 1000) : 3600 }
      )
    }
...
export async function trackShareOpen(shareToken: string, metadata: Record<string, any>) {
...
export function generateDeepLink(entityType: EntityType, entityId: string): string {
