...
export class CommentController {
  static async postComment(req: Request, res: Response) {
    ...
    if (!Object.values(EntityType).includes(entityType)) return res.status(400).json({ error: 'Invalid entityType' })
    if (!contentText || contentText.length > 250) return res.status(400).json({ error: 'Comment too long' })
    const sanitizedText = sanitizeInput(contentText)
    const comment = await prisma.$transaction(async (tx) => {
      const newComment = await tx.comment.create({
        data: { userId, entityType, entityId, contentText: sanitizedText, parentCommentId }
      })
  ...
  static async deleteComment(req: Request, res: Response) {
    ...
    if (!comment || comment.userId !== userId) return res.status(403).json({ error: 'Forbidden' })
